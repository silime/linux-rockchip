...
static int dhdpcie_download_buffer(dhd_bus_t *bus, FWPKG_FILE *fw, fwpkg_info_t *fwpkg,
	uint32 unit_type, uint32 buf_size, uint32 addr);
...
dhdpcie_chip_specific_init(dhd_bus_t *bus, uint chipid)
{
	...
	if (BCM4349_CHIP(chipid) || BCM4350_CHIP(chipid) || BCM4345_CHIP(chipid)) {
		...
		DHD_PRINT(("Disable CTO for chip 0x%x\n", chipid));
		bus->cto_enable = FALSE;
	} else {
...
dhdpcie_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
	...
	buf_len = fwpkg_open_firmware_img(&fw, fwpkg, FWPKG_TAG_FW, pfw_path,
		__FUNCTION__);
	if (buf_len <= 0) {
	...
	dhd_print_fw_ver_from_file(bus->dhd, fw, fwpkg, buf_len);
#endif /* TAG_INFO_FW_VER */
	bus->fw_download_len = buf_len;
	bus->fw_download_addr = bus->dongle_ram_base;
	bcmerror = dhdpcie_download_buffer(bus, fw, fwpkg, FWPKG_TAG_FW, buf_len,
...
static int
dhdpcie_download_buffer(dhd_bus_t *bus, FWPKG_FILE *fw, fwpkg_info_t *fwpkg,
	uint32 unit_type, uint32 buf_size, uint32 addr)
{
	...
	memptr_w = MALLOC(bus->dhd->osh, MEMBLOCK);
	if (memptr_w == NULL) {
	...
	if (bus->dhd->conf->fwchk) {
		memptr_r = MALLOC(bus->dhd->osh, MEMBLOCK);
		if (memptr_r == NULL) {
			DHD_ERROR(("%s: Failed to allocate memory %d bytes\n", __FUNCTION__, MEMBLOCK));
			goto err;
		}
	...
	while (residual_len) {
		...
		len = fwpkg_get_firmware_img_block(fw, fwpkg, unit_type,
			memptr_w, len, buf_offset);
		if (len <= 0) {
		...
		if (store_reset) {
			ASSERT(offset == 0);
			bus->resetinstr = *((uint32 *)memptr_w);
...
#ifdef CHECK_DOWNLOAD_FW
		if (bus->dhd->conf->fwchk) {
			bcmerror = dhdpcie_bus_membytes(bus, FALSE, DHD_PCIE_MEM_BAR1, offset,
				memptr_r, len);
			if (bcmerror) {
			...
			if (memcmp(memptr_r, memptr_w, len)) {
...
err:
	if (memptr_w)
		MFREE(bus->dhd->osh, memptr_w, MEMBLOCK);
#ifdef CHECK_DOWNLOAD_FW
	if (memptr_r)
		MFREE(bus->dhd->osh, memptr_r, MEMBLOCK);
#endif
	return bcmerror;
}
...
dhdpcie_bus_download_ram_bootloader(dhd_bus_t *bus)
{
	...
	len = fwpkg_open_firmware_img(&fw, &fwpkg, FWPKG_TAG_ZERO, bus->bootloader_filename,
		__FUNCTION__);
	if (len <= 0) {
		DHD_ERROR("Ignore bootloader file %s\n", bus->bootloader_filename);
		goto err;
	}

	ret = dhdpcie_download_buffer(bus, fw, fwpkg, FWPKG_TAG_ZERO, len,
			bus->bootloader_addr);
	if (ret < 0) {
		DHD_ERROR(("%s: dhdpcie_download_buffer error : %d\n", __FUNCTION__, ret));
		goto err;
...
dhdpcie_bus_write_vars(dhd_bus_t *bus)
{
	...
	DHD_PRINT(("%s: New varsize is %d, length token(nvram_csm)=0x%08x\n", __FUNCTION__, varsize, varsizew));
...
/* In phones sometimes NOC is seen for any further access after readshared fails */
	...
	if ((addr == 0) || (addr == bus->nvram_csm) || (addr < bus->dongle_ram_base) ||
		...
		DHD_PRINT(("%s: address (0x%08x) of pciedev_shared invalid\n",
			__FUNCTION__, addr));
		DHD_PRINT(("%s: Waited %llu usec, dongle is not ready\n", __FUNCTION__, elapsed));
		...
		*/
		...
		DHD_PRINT(("### Total time ARM OOR to Readshared pass took %llu usec ###\n",
			DIV_U64_BY_U32((bus->rd_shared_pass_time - bus->arm_oor_time),
				NSEC_PER_USEC)));
		DHD_PRINT(("%s: PCIe shared addr (0x%08x) read took %llu usec "

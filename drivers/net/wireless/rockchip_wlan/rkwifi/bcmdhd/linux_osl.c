...
{
	bool invert_s = FALSE, invert_us = FALSE;
	uint32 diff_s, diff_us;

	invert_s = (old_ts->tv_sec > cur_ts->tv_sec) ? TRUE : FALSE;
	invert_us = (old_ts->tv_usec > cur_ts->tv_usec) ? TRUE : FALSE;

	diff_s = invert_s ? ((uint32)old_ts->tv_sec - (uint32)cur_ts->tv_sec) :
		((uint32)cur_ts->tv_sec - (uint32)old_ts->tv_sec);
	diff_us = invert_us ? (old_ts->tv_usec - cur_ts->tv_usec) :
		(cur_ts->tv_usec - old_ts->tv_usec);

	if (invert_s ^ invert_us)
		diff_us = diff_s * 1000000 - diff_us;
	else
		diff_us = diff_s * 1000000 + diff_us;
